...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(
                        FullAttention(False, configs.factor, configs.order, configs.dropout,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, x_mask_enc=None, x_mask_dec=None):
        ...
        x_fft = torch.fft.fft(x_enc, dim=1)
        am = torch.abs(x_fft)
        _, top_indicies = torch.topk(am, k=24, dim=1)
        mask = torch.zeros_like(x_fft)

        for i in range(B):
          mask[i, top_indicies[i], :] = 1
        x1 = x_fft * mask
        x1 = torch.fft.ifft(x1, dim=1).real

        x2 = x_enc - x1
        # Embedding
        enc_out1 = self.enc_embedding(x1, x_mark_enc)
        enc_out2 = self.enc_embedding(x2, x_mark_enc)
        # Encoder
